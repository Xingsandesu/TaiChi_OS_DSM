...
docker_install_taichi() {
	...
	mkdir -p /volume1/docker/taichi
	mkdir -p /volume1/docker/taichi_app
	touch /volume1/docker/taichi/config.json
	touch /volume1/docker/taichi/data.db
	echo "目录创建完成"
...
uninstall_taichi() {
	...
	rm -rf /volume1/docker/taichi || true
...
case $operation in
	...
	5)
		# 恢复默认设置
		echo "开始恢复默认设置..."
		rm -rf /volume1/docker/taichi/config.json
		docker restart taichios
		echo "恢复默认设置完毕"
		;;
	6)
		# 重置账号密码
		echo "开始重置账号密码..."
		rm -rf /volume1/docker/taichi/data.db
		docker restart taichios
		echo "重置账号密码完毕"
		;;
	7)
	...
	8)
		...
		if [ -f "/volume1/docker/taichi/config.json" ]; then
